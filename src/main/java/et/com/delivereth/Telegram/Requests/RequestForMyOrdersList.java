...
public class RequestForMyOrdersList {
    ...
    public void requestForMyOrdersList(Update update, TelegramUserDTO telegramUser) {
        ...
        orderList.toList().forEach(restorantDTO -> {
            sendMyOrders(restorantDTO, update);
        });
        if (orderList.toList().size() == 0 || !orderList.hasNext()) {
    ...
    private void sendMyOrders(OrderDTO orderDTO, Update update) {
        ...
        if (orderDTO.getOrderStatus().equals(OrderStatus.CANCELED_BY_RESTORANT) ||
        ...
        if (orderDTO.getOrderStatus().equals(OrderStatus.WAITING) ||
        ...
        String invoice = "<strong>Restaurant Name: " +
            dbUtility.getRestorant(dbUtility.getFood(orderedFoodList.get(0).getId()).getId()).getName() +
            "</strong>\n";
        Double total = 0D;
